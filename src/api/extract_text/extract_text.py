...
def extract_text_from_pdf(file_path: str = Query(..., description="The path to the PDF file")):
    """
    ...
    if not file_path.endswith('.pdf') or not os.path.exists(file_path):
        raise HTTPException(status_code=400, detail="Invalid file format or file does not exist. Please provide a valid PDF file path.")

    try:
        with open(file_path, "rb") as file:
            extracted_texts = extract_text_from_image(file.read())
        return JSONResponse(content={"extracted_texts": extracted_texts}, status_code=200)
    except ValueError as ve:
        raise HTTPException(status_code=500, detail=str(ve))


@router.get("/extract-text-word")
def extract_text_from_word(file_path: str = Query(..., description="The path to the Word file")):
    """
